...
def darkroom2Action(answer):
    ...
    if answer == "second chest":
        ...
        state["location"] = "darkroom2"
        state["prompt"] = "Do you don the armor?\n"
    ...
    if answer == "put it on":
        if "Half Chain" not in state["inventory"]:
            state["inventory"].append("Half Chain")
            cprint(f"Inventory:{state['inventory']}", "green")
            state["armor"] = 16
            cprint(f"Armor:{state['armor']}", "cyan")
            ...
            state["allowed_resps"] = ["chest", "pile of clothes", "window", "door"]
